...
parser.add_argument('--JOB-NAME', type=str, default='Grid_searcher',
        help='the name of hdfs process')
...
lunch_time  = time.strftime("date_%Y_%m_%d_time_%H_%M_%S", time.localtime()) #time stamp, used to avoid filename collision
lr_factor   = "0.1"
lr_factor_epoch = "3,8"
...
def lunch_test(lr_factor = "1", lr_factor_epoch = "1"):
    ...
    command = ("python train_speech.py" + 
            " --model-dir "         + args.model_dir + '/' + str(lunch_time) + '_id_' + str(test_cnt) + "/ "
            ...
            " 2>&1 ")
    submit = ("../.submit.sh " +
            " --pods "      + str(len(args.gpus.split(','))) +
            " --JOB_NAME "  + args.JOB_NAME + '_' + str(lunch_time) + '_id_' + str(test_cnt))

    open("lunch_thread.sh", "w").write(command)
    child  = subprocess.Popen(submit, shell = True)
    child.wait()
